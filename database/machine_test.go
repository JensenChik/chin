...
func TestMachine(t *testing.T) {
    ...
    g.Describe("测试 machine 表更新", func() {
        ...
        g.Xit("记录被正确更新", func() {
            ...
            for _, machine := range machines {
                ...
                db.Model(new(Machine)).Where("task_id = ?", machine.CPULoad).Count(&mysqlCount)
                g.Assert(mysqlCount).Equal(1)
                ...
                newMachineName := randomString(3)
                machine.MachineName = newMachineName
                machine.DumpToMySQL()

                newMachine, _ := new(Machine).LoadByWhere("task_id = ?", machine.CPULoad)
    ...
    g.Describe("测试 machine 数据加载", func() {
        ...
        g.It("记录通过where条件被正确加载", func() {
            for _, machine := range machines {
                newMachine, err := new(Machine).LoadByWhere("task_id = ?", machine.CPULoad)
                g.Assert(err == nil).IsTrue()
                g.Assert(newMachine.MachineName).Equal(machine.MachineName)
            }
        })
        ...
        g.It("当存在多于一条记录满足where条件时无法实例化，返回异常且对象为nil", func() {
            (&Machine{CPULoad:12580, MachineName:randomString(20)}).DumpToMySQL()
            (&Machine{CPULoad:12580, MachineName:randomString(20)}).DumpToMySQL()
            machine, err := new(Machine).LoadByWhere("task_id = ?", 12580)
            g.Assert(machine == nil).IsTrue()
            g.Assert(err.Error()).Equal("存在多条满足条件的记录，无法实例化")
        })

        g.It("当存在零条记录满足where条件时无法实例化，返回异常且对象为nil", func() {
            newMachine, err := new(Machine).LoadByWhere("task_id = ?", 999999)
